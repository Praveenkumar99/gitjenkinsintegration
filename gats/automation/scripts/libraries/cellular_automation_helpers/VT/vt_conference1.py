#Vt_Call Ongoing call A to B and add call from A to C
import time
import logging
...
class perform_vt_conf_call:

    def __init__(self, tst, yamlData):
        self.tst = tst
        self.Data = yamlData

    def execute_conference_call(self):
        def __init__(self, tst, yamlData):
        self.tst = tst
        self.Data = yamlData

    def execute_vt_conf_call(self):
        ...
        log.info("Performing Volte call in device A to device C")
        ...
        # fetching the call state of Device C
        status, mCallState = adb.checking_call_state(
        ...
        if mCallState != adb.CallStates.INCOMING_CALL.value:
            return False, "Call not received in device C"
        ...
        log.info("Call attended successfully in device C")
        ...
        status, msg = tap_command(self.Data['serialId'], str(coordinates[0]), 
            str(coordinates[1]))
...
def vt_conferece_call1(tst, yamlData):
    status, msg, noOfDevices = cf.comp_id(yamlData['testcase_config'][tst]["tempDevicesId"], gc.IMAGE_FOLDER)
    if noOfDevices >= 3:
        call_obj = perform_vt_conf_call(tst, yamlData)
        status, msg = call_obj.execute_vt_conf_call()
    else:
