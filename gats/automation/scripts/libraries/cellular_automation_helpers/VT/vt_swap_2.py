...
class SwappingCall:
    ...
    def execute_call(self):
        ...
        log.info("VT Call attended successfully in device B")

        time.sleep(1)

        log.info("==Making a VT call from C to A keeping A and B VT call on hold==")
        # Triggering the Call from Device C to A
        status, msg = adb.trigger_vt_call(
        ...
        # Receiving the VT Call in Devices A from devices C
        if mCallState != adb.CallStates.CONNECTED_CALL.value:
            return False, "VT Call not received in devices A"

        log.info("Receiving VT call in device A")
        status, msg = adb.accept_call(deviceId=self.Data['testcase_config'][self.tst]['tempDevicesId'][1])

        if not status:
            return False, msg

        log.info("VT Call attended successfully in devices A")
        ...
        # Disconnecting the Call A to C call
...
def vt_call_swap_2(tst, yamlData):
