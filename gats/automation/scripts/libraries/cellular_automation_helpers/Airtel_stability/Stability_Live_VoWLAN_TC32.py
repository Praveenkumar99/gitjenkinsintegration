...
class Vowlan:
    ...
    def test_for_heat(self):
        ...
        if self.Data[self.tst].get('passwd') is None:
            cf.execute_commands(
                f'adb -s {self.Data["serialId"]} shell am start -n com.steinwurf.adbjoinwifi/.MainActivity -e ssid {self.Data["testcase_config"][self.tst]["SSID"]}')
        else:
            cf.execute_commands(f'adb -s {self.Data["serialId"]} shell am start -n com.steinwurf.adbjoinwifi/.MainActivity\
                    -e ssid {self.Data["testcase_config"][self.tst]["SSID"]} '
                                f'-e password_type WEP|WPA -e password {self.Data["testcase_config"][self.tst]["passwd"]}')
        ...
        return True, "VoWlAN VT Call Performed Successfully "


def volwan_video(tst, yamlData):
