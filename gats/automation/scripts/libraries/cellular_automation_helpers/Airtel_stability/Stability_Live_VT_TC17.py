...
class SwappingVTCall:
    ...
    def execute_call(self):
        log.info("==Performing Swap on A2B and A2C VT call==")

        log.info("==Making a VT call from A to B==")
        # Triggering the VT Call from Device A
        ...
        # fetching the VT call state of Devices B
        ...
        # Receiving the VT Call in Devices B
        ...
        log.info("==Making a VT call from A to C keeping A and B VT call on hold==")
        # Triggering the VT Call from Device A to C
        ...
        if mCallState != adb.CallStates.INCOMING_CALL.value:
            return False, "VT Call not received in devices C"

        log.info("Receiving VT call in device C")
        status, msg = adb.accept_call(deviceId=self.Data['testcase_config'][self.tst]['tempDevicesId'][1])

        if not status:
            return False, msg

        log.info("VT Call attended successfully in devices C")
        ...
        while True:
            ...
            log.info(f"Swap performed successfully")

        # Disconnecting the Call A to B call
        log.info("Terminating A to B VT call in device A")
        status, msg = adb.terminate_call(deviceId=self.Data['serialId'])
        if not status:
            return False, msg

        # Disconnecting the Call A to B call
        log.info("Terminating A to C VT call in device A")
        status, msg = adb.terminate_call(deviceId=self.Data['serialId'])
        if not status:
            return False, msg

        return True, "VT Called Swapped Successfully in Device A"
