#During vt call send SMS from MT side and Receive MO side
import time
import logging
...
class perform_vt_call_send_sms:

    def __init__(self, tst, yamlData):
        self.tst = tst
        self.Data = yamlData

    def execute_vt_call_send_sms(self):
        ...
        # Sending SMS   
        for loop in range(5):
            log.info(f"Sending sms from device A {loop} times")
...
def vt_sms_2(tst, yamlData):
    status, msg, noOfDevices = cf.comp_id(yamlData['testcase_config'][tst]["tempDevicesId"])
    if noOfDevices >= 2:
        call_obj = perform_vt_call_send_sms(tst, yamlData)
        status, msg = call_obj.execute_vt_call_send_sms()
    else:
...
class perform_vt_call_send_sms:
    ...
    def execute_vt_call_send_sms(self):
        ...
        log.info("Sending sms from device B")
        status, msg = adb.send_sms(
            deviceId=self.Data['testcase_config'][self.tst]["tempDevicesId"][0],
            phoneNumber=self.Data['testcase_config'][self.tst]["callA_no"])
